...
/**
 * Summer 2012: Added minor changes for reddit video watching app.
...
public class LruCache<K, V>
{
   ...
   public final V get(K key)
   {
       ...
       * be different when create() returns. If a conflicting value was added to
       * the map while create() was working, we leave that value in the map and
   ...
   private void trimToSize(int maxSize)
   {
      while (true)
      {
         ...
         synchronized (this)
         {
            ...
            // NO eldest() METHOD IN LINKEDHASHMAP!//
            Map.Entry<K, V> toEvict;
            Iterator<Map.Entry<K, V>> entryIter = map.entrySet().iterator();

            if (!(entryIter.hasNext()) || (toEvict = entryIter.next()) == null)
            {
               break;
            }
    ...
    *           removal was caused by a {@link #put}. Otherwise it was caused by
    ...
    * of entries in the cache. For all other caches, this returns the sum of the
   ...
   /**
    * Returns the number of times {@link #get} returned a value that was already
